...
namespace Aplicacion.PlanesTratamiento
{
    public class ConsultaUltimoCodigo
    {
        ...
        public class Manejador : IRequestHandler<Listar, object>
        {
            ...
            {
                string codigoTratamiento = _context.Tratamiento.OrderByDescending(ev => ev.Codigo).FirstOrDefault().Codigo;
                string output = "";
                if (codigoTratamiento == null)
                {
                    output = EntityCodes.Tratamiento + "001";
                }
                else
                {
                    string substr = codigoTratamiento.Substring(2, 3);
                    output = EntityCodes.Tratamiento + (int.Parse(substr) + 1).ToString().PadLeft(substr.Length, '0');
                }
